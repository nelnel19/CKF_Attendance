...
const UserLists = () => {
  ...
  const fetchConsolidationRecords = async () => {
    setConsolidationLoading(true);
    try {
      const response = await axios.get(CONSOLIDATION_URL);
      setConsolidationRecords(response.data.data);
      setConsolidationError('');
    } catch (err) {
      setConsolidationError(err.response?.data?.message || 'Failed to fetch consolidation records');
    } finally {
  ...
  const filteredConsolidation = consolidationRecords.filter(record => {
    let matchSearch = true;
    let matchProgress = true;

    if (consolidationSearch) {
      matchSearch = record.fullName.toLowerCase().includes(consolidationSearch.toLowerCase());
    }

    if (consolidationFilterProgress !== 'all') {
      ...
      if (consolidationFilterProgress === 'completed') {
        ...
        matchProgress = progress > 0 && progress < 100;
      } else if (consolidationFilterProgress === 'not-started') {
  ...
  const generateConsolidationSummary = () => {
    setSummaryType('consolidation');
    const summary = {
      total: consolidationRecords.length,
      byLesson: {},
      overallProgress: 0
    };

    consolidationColumns.forEach(col => {
      const completed = consolidationRecords.filter(record => record[col] === true).length;
      summary.byLesson[col] = completed;
    });

    const totalCompletions = consolidationRecords.reduce((total, record) => {
      return total + consolidationColumns.filter(col => record[col] === true).length;
    }, 0);

    const totalPossible = consolidationRecords.length * consolidationColumns.length;
    summary.overallProgress = totalPossible > 0 ? Math.round((totalCompletions / totalPossible) * 100) : 0;
  ...
  const exportSummaryToExcel = () => {
    ...
    if (summaryType === 'attendance') {
      ...
      summaryRows = [
        ['CKF CONSOLIDATION PROGRESS REPORT'],
        [''],
        [`Generated: ${currentDate}`],
        [''],
        ['OVERALL STATISTICS'],
        ['Total Members in Consolidation', summaryData.total],
    ...
    if (window.confirm(`Are you sure you want to delete ${fullName}?`)) {
      try {
        if (type === 'attendance') {
          ...
          await axios.delete(`${CONSOLIDATION_URL}/${id}`);
          fetchConsolidationRecords();
        }
      } catch (err) {
        if (type === 'attendance') {
          ...
          setConsolidationError(err.response?.data?.message || 'Failed to delete consolidation record');
        }
      }
    }
  };
  ...
  const toggleMemberSelection = (member) => {
    setSelectedMembers(prev => {
      if (prev.find(m => m._id === member._id)) {
  ...
  const handleAddToConsolidation = async () => {
    if (selectedMembers.length === 0) {
      setConsolidationAddError('Please select at least one member.');
      return;
    }

    setAddingConsolidation(true);
    setConsolidationAddError('');
    ...
    try {
      const records = selectedMembers.map(member => ({
        ...
        Lordship: false,
        Forgiveness: false,
        '4 Greatest Meetings': false,
        Devotion: false,
        Prayer: false,
        'Witnessing Testimony': false,
        'Life of Obedience': false,
        'Life in Church': false
      }));

      await axios.post(`${CONSOLIDATION_URL}/batch`, { records });
      await fetchConsolidationRecords();
      setIsConsolidationModalOpen(false);
      setSelectedMembers([]);
    } catch (err) {
      setConsolidationAddError(err.response?.data?.message || 'Failed to add members to consolidation');
    } finally {
      setAddingConsolidation(false);
    }
  };

  const toggleLessonCompletion = async (recordId, lessonKey) => {
    try {
      const record = consolidationRecords.find(r => r._id === recordId);
      if (!record) return;
      ...
      setConsolidationRecords(prev =>
        prev.map(r => r._id === recordId ? { ...r, [lessonKey]: updatedValue } : r)
      );
    } catch (err) {
      setConsolidationError(err.response?.data?.message || 'Failed to update lesson progress');
    }
  };

  const getProgressPercentage = (record) => {
    const completed = consolidationColumns.filter(col => record[col] === true).length;
    return Math.round((completed / consolidationColumns.length) * 100);
  };
  ...
  const exportConsolidationToExcel = () => {
    const exportData = filteredConsolidation.map(record => {
      const row = {
        'Full Name': record.fullName,
        'Gender': record.gender || '-',
        'Age': record.age,
        'Age Category': getAgeCategory(record.age),
        'Address': record.address,
        'Contact No': record.contactNo,
        'Cellgroup Leader': record.cellgroupLeader,
    ...
    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'CKF_Consolidation');
    const date = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
    const fileName = `CKF_Consolidation_${date}.xlsx`;
    XLSX.writeFile(wb, fileName);
  };
  ...
  const clearConsolidationFilters = () => {
    setConsolidationSearch('');
  ...
  return (
    <div className="user-list-container">
      ...
      {activeTab === 'consolidation' && (
        <>
          <div className="controls">
            ...
            <div className="filter-group">
              <div className="filter">
                <label htmlFor="consolidationSearch">Search:</label>
                <input
                  type="text"
                  id="consolidationSearch"
              ...
              <div className="filter">
                <label htmlFor="progressFilter">Progress:</label>
                <select
                  id="progressFilter"
                  value={consolidationFilterProgress}
                  onChange={(e) => setConsolidationFilterProgress(e.target.value)}
                >
              ...
              {(consolidationSearch || consolidationFilterProgress !== 'all') && (
                <button onClick={clearConsolidationFilters} className="clear-filters-btn">
                  Clear Filters
                </button>
              )}

              <button className="summary-btn" onClick={generateConsolidationSummary}>
                Generate Summary
              </button>

              {filteredConsolidation.length > 0 && (
                <button className="export-btn" onClick={exportConsolidationToExcel}>
                  Export to Excel
                </button>
              )}
            </div>
          ...
          {consolidationError && <div className="error">{consolidationError}</div>}
          ...
          {filteredConsolidation.length === 0 ? (
            ...
            <div className="table-wrapper">
              <div className="table-scroll-container">
                <table className="user-table consolidation-table">
                  <thead>
                    <tr>
                      <th className="sticky-col">Full Name</th>
                      <th className="sticky-col">Progress</th>
                      {consolidationColumns.map(col => (
                        <th key={col} className="lesson-col">{col}</th>
                      ))}
                  ...
                  <tbody>
                    {filteredConsolidation.map(record => (
                      <tr key={record._id}>
                        ...
                        {consolidationColumns.map(col => (
                          <td key={col} className="lesson-col">
                            <label className="checkbox-container">
                              <input
                                type="checkbox"
                                checked={record[col] === true}
                                onChange={() => toggleLessonCompletion(record._id, col)}
                              />
            ...
            <div className="summary-body">
              ...
              {summaryType === 'attendance' ? (
                ...
                <>
                  ...
                  <div className="summary-section">
                    <h3>Lesson Completion Breakdown</h3>
                    <table className="summary-table">
                      <thead>
                        <tr>
                          <th>Lesson</th>
                          <th>Members Completed</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(summaryData.byLesson).map(([lesson, count]) => (
                          <tr key={lesson}>
                            <td>{lesson}</td>
                            <td>{count}</td>
                          </tr>
      ...
      {/* Consolidation Add Members Modal */}
      {isConsolidationModalOpen && (
        <div className="modal-overlay" onClick={() => setIsConsolidationModalOpen(false)}>
          <div className="modal-content add-member-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div>
                <h2>Add Members to Consolidation</h2>
                <p className="modal-subtitle">Select members from the directory to add to consolidation tracking</p>
              </div>
